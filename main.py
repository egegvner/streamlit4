...
def admin_panel(conn):
    ...
    if st.button("Update News", use_container_width = True):
        for _, row in edited_df.iterrows():
            c.execute("UPDATE news SET title = ?, content = ?, likes = ?, dislikes = ?, created = ?, category = ? WHERE news_id = ?", (row["Title"], row["Content"], row["Likes"], row["Dislikes"], row["Category"], row["Category"], row["ID"]))
        conn.commit()
        st.rerun()
