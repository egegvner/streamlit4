    ...
    if transactions:
        for t in transactions:
            ...
            if t_type == "Top Up":
                ...
                st.write(f"Amount   $|$   :green[+${format_currency(int(amount))}]")
                st.write(f"New Balance   $|$   :green[${format_currency(int(balance))}]")
                st.text("")
                st.text("")
                st.caption(timestamp)
                ...
                st.write(f"Amount   $|$   :green[+${format_currency(int(amount))}]")
                st.write(f"New Savings Balance   $|$   :green[${format_currency(int(balance))}]")
                st.text("")
                st.text("")
                st.caption(timestamp)
                ...
                st.write(f"Amount   $|$   :red[-${format_currency(int(amount))}]")
                st.write(f"Remaining Balance   $|$   :green[${format_currency(int(balance))}]")
                st.text("")
                st.text("")
                st.caption(timestamp)
            else:
                st.warning(f"Other Transaction $|$ (Status: {status})")
                st.write(f"Amount   $|$   ${format_currency(int(amount))}")
                st.write(f"Balance   $|$   ${format_currency(int(balance))}")
                st.text("")
                st.text("")                
                st.caption(timestamp)
